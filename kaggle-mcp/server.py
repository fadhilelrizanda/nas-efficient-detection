...
def _is_cli_error(text: str) -> bool:
    return text.startswith("[exit code") or text.startswith("[timeout") or text.startswith("[error]")
...
def _read_kernel_ref_from_metadata(path: Optional[str]) -> str | None:
    kernel_dir = Path(path or ".")
    ...
    return kernel_ref if isinstance(kernel_ref, str) and "/" in kernel_ref else None


def _extract_kernel_ref_from_push_output(output: str) -> str | None:
    patterns = [
...
def _extract_last_metric(text: str, patterns: list[str]) -> tuple[str | None, str | None]:
...
def _extract_training_metrics(log_text: str) -> dict[str, str | None]:
    epoch, epoch_line = _extract_last_metric(
        log_text,
        [
            r"\bepoch\s*[:=#/]?\s*(\d+(?:\s*/\s*\d+)?)",
            r"\bepoch\s+\[(\d+(?:\s*/\s*\d+)?)\]",
        ],
    )
    loss, loss_line = _extract_last_metric(
        log_text,
        [
            r"\b(?:train[_\s-]*)?loss\s*[:=]\s*([-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?)",
            r"\bloss\s+([-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?)",
            r"\bval[_\s-]*loss\s*[:=]\s*([-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?)",
        ],
    )
    return {
        "epoch": epoch,
...
def _infer_training_condition(status_text: str, log_text: str) -> str:
    combined = f"{status_text}\n{log_text}".lower()

    if any(token in combined for token in ["traceback", "exception", "runtimeerror", "error", "failed"]):
        return "unhealthy"
    if any(token in combined for token in ["stopped", "cancelled", "canceled", "aborted", "killed"]):
        return "stopped"
    if any(token in combined for token in ["complete", "completed", "finished", "succeeded", "success"]):
        return "completed"
    if any(token in combined for token in ["running", "processing", "queued", "starting"]):
        return "healthy"
    if re.search(r"\b(epoch|loss|val_loss|accuracy|step)\b", log_text, flags=re.IGNORECASE):
        return "healthy"
    if "(empty response)" in log_text:
        return "starting"
    return "unknown"


def _build_training_summary(kernel: str, status_text: str, log_text: str) -> str:
    metrics = _extract_training_metrics(log_text)
    condition = _infer_training_condition(status_text, log_text)
    lines = [
        f"kernel: {kernel}",
        f"condition: {condition}",
        f"status: {status_text.strip() or '(empty response)'}",
        f"current_epoch: {metrics['epoch'] or 'unknown'}",
        f"current_loss: {metrics['loss'] or 'unknown'}",
    ]
    if metrics["epoch_line"]:
        lines.append(f"epoch_line: {metrics['epoch_line']}")
    if metrics["loss_line"] and metrics["loss_line"] != metrics["epoch_line"]:
        lines.append(f"loss_line: {metrics['loss_line']}")
    return "\n".join(lines)


def _collect_kernel_training_snapshot(kernel: str, lines: int = 50) -> str:
    lines = max(1, min(lines, 300))
    status_text = _run_as(["kaggle", "kernels", "status", kernel], timeout=120)
    log_text = _run_as(["kaggle", "kernels", "logs", kernel], timeout=120)

    if _is_cli_error(status_text):
        return status_text
    if _is_cli_error(log_text):
        return log_text

    summary = _build_training_summary(kernel, status_text, log_text)
    tail = _tail_text(log_text, lines)
    return summary + "\n\nRecent logs\n" + tail


def _follow_kernel_logs(kernel: str, follow_timeout: int = 900, interval: Optional[int] = None) -> str:
    cmd = ["kaggle", "kernels", "logs", kernel, "--follow"]
    if interval is not None:
        cmd.extend(["--interval", str(interval)])
    return _run_as(cmd, timeout=follow_timeout)
...
def kaggle_kernels_logs_tail(
    ...
    Use this tool for cheap polling when you only need a snapshot. For
    immediate training inspection, prefer `kaggle_kernels_logs_live` or
    `kaggle_kernels_push_live`.
    ...
    if _is_cli_error(raw):
        return raw

    return _tail_text(raw, lines)


@mcp.tool()
def kaggle_kernels_logs_live(
    kernel: str,
    follow_timeout: int = 900,
    interval: Optional[int] = None,
    lines: int = 80,
) -> str:
    """Follow live kernel logs and summarize training health.

    This tool uses `kaggle kernels logs --follow` so the agent can inspect
    training immediately after a run starts. The command is bounded by
    `follow_timeout`; when the timeout is reached, partial logs gathered so
    far are still returned.

    Args:
        kernel: Kernel URL suffix in format <owner>/<kernel-name>.
        follow_timeout: Maximum time to wait while following logs.
        interval: Polling interval in seconds for follow mode.
        lines: Number of trailing lines to include in the returned log view.
    """
    lines = max(1, min(lines, 300))
    status_text = _run_as(["kaggle", "kernels", "status", kernel], timeout=120)
    if _is_cli_error(status_text):
    ...
    live_logs = _follow_kernel_logs(kernel, follow_timeout=follow_timeout, interval=interval)
    if _is_cli_error(live_logs) and not live_logs.endswith(f"[timeout after {follow_timeout}s]"):
        return live_logs

    summary = _build_training_summary(kernel, status_text, live_logs)
    tail = _tail_text(live_logs, lines)
    return summary + "\n\nLive logs\n" + tail


@mcp.tool()
def kaggle_kernels_training_status(
    kernel: str,
    lines: int = 80,
) -> str:
    """Summarize current kernel training state from status and logs.

    Returns a concise health summary plus the latest parsed epoch and loss.
    Use this after `kaggle_kernels_push_live` or when you need a fresh
    snapshot without following logs.

    Args:
        kernel: Kernel URL suffix in format <owner>/<kernel-name>.
        lines: Number of trailing log lines to include.
    """
    return _collect_kernel_training_snapshot(kernel, lines=lines)


@mcp.tool()
def kaggle_kernels_push_live(
    path: Optional[str] = None,
    timeout: Optional[int] = None,
    accelerator: Optional[str] = None,
    follow_timeout: int = 900,
    ...
    """Push a kernel and immediately follow live logs.

    This is the concrete training workflow tool for agents: push the kernel,
    resolve the kernel id, then start `kaggle kernels logs --follow` right
    away so the run condition, current epoch, and current loss can be
    inspected without a second tool call.

    Args:
        path: Folder for upload, containing data files and kernel-metadata.json.
        timeout: Maximum run time in seconds for the kernel.
        accelerator: Accelerator type (e.g. 'GPU', 'TPU').
        follow_timeout: Maximum time to wait while following live logs.
        interval: Polling interval in seconds for follow mode.
        lines: Number of trailing live-log lines to include in the return.
    """
    cmd = ["kaggle", "kernels", "push"]
    if path:
        cmd.extend(["-p", path])
    ...
    push_output = _run_as(cmd, timeout=300)
    if _is_cli_error(push_output):
        return push_output

    kernel = _read_kernel_ref_from_metadata(path) or _extract_kernel_ref_from_push_output(push_output)
    if not kernel:
        return push_output + "\n\n[warning] Push succeeded but the kernel id could not be resolved. Run kaggle_kernels_logs_live(kernel=...) manually."

    live_summary = kaggle_kernels_logs_live(
        kernel=kernel,
        follow_timeout=follow_timeout,
        interval=interval,
        lines=lines,
    )
    return f"Push output\n{push_output}\n\n{live_summary}"
